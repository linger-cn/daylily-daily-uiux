...
## 3. `prototypes/` 规范（与 AGENTS.md 一致）

- **项目代号目录**：`prototypes/<project_code>/`，`project_code` 为小写字母、数字、连字符（kebab-case），例如 `daylily-daily`。
- **推荐自治结构**：每个项目代号目录内宜包含 `prd/`、`styles/`、`assets/`、`index.html` 及若干语义化页面 `*.html`。具体以 [AGENTS.md](./AGENTS.md) 为准。
- **资源引用**：
...
- **新增产品线** 时：新建 `prototypes/<new-project-code>/`，按需初始化 `styles/`、`assets/`，并在 README 或索引页中登记入口（如 `index.html`）。
